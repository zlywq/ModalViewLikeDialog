...
class DialogList_LikeTree : UIView , UITableViewDataSource, UITableViewDelegate {
    ...
    var m_keyName : String = "name"
    var m_keyValue : String = "value"
    var m_keyLevel : String? = "level"
    var m_items : Array<Dictionary<String,AnyObject>>? = nil
    ...
    func init2Data(items:Array<Dictionary<String,AnyObject>>, keyName:String, keyValue:String, keyLevel:String?, getResultBeforeHide: ((val:AnyObject)->Void)?){
    ...
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        var cnt = 0;
        cnt = m_items==nil ? 0 : m_items!.count
        return cnt
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        var item : Dictionary<String,AnyObject> = m_items![indexPath.row]
        var name:String! = item[m_keyName] as String
        var level:Int = 0
        if (m_keyLevel != nil){
            var level2 = item[m_keyLevel!] as? Int
            if (level2 != nil){
                level = level2!
            }
        }
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath){
        if (getResultBeforeHide != nil){
            var item : Dictionary<String,AnyObject> = m_items![indexPath.row]
            var val: AnyObject! = item[m_keyValue]
